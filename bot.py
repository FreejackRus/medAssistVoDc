...
OLLAMA_MODEL: str = "hf.co/unsloth/Qwen3-30B-A3B-Instruct-2507-GGUF:IQ4_XS"  # полное имя после ollama pull
...
class MedicalAssistant:
    ...
    def _generate_streaming(self, sections: Dict[str, str], file: TextIO) -> None:
        ...
        system_prompt = (
            ...
            "Ты — русскоязычный медицинский ассистент, специализирующийся на клинических рекомендациях. "
            ...
            "Используй медицинскую терминологию точно, как в оригинальном документе. "
            "Будь максимально точным и детальным в медицинских аспектах. "
        ...
        **Клинические рекомендации:**
        ...
        try:
            stream = ollama.chat(
                ...
                stream=True,
                options={
                    # Оптимизированные параметры для Qwen3-30B-A3B-Instruct-2507-GGUF
                    "temperature": 0.3,  # Увеличено с 0.1 для более естественных ответов
                    "top_p": 0.9,        # Снижено с 0.95 для более фокусированной генерации
                    "top_k": 40,         # Добавлен параметр для контроля разнообразия
                    "repeat_penalty": 1.1, # Добавлен для предотвращения повторений
                    "num_predict": -1,   # Без ограничений на длину ответа
                    "num_ctx": 200000,   # Увеличен контекст под возможности модели
                    "stop": ["<|im_end|>", "</s>", "<|endoftext|>", "Human:", "Assistant:"],  # Расширенные stop-токены для Qwen3
    ...
    def _generate_dialogue_streaming(self, user_message: str, conversation_history: List[Dict], sections: Dict[str, str], file: TextIO) -> None:
        ...
        try:
            ...
            stream = ollama.chat(
                ...
                stream=True,
                options={
                    # Оптимизированные параметры для диалогового режима
                    "temperature": 0.4,  # Немного выше для более живого диалога
                    "top_p": 0.85,       # Снижено для более точных ответов
                    "top_k": 50,         # Увеличено для диалога
                    "repeat_penalty": 1.15, # Выше для предотвращения повторений в диалоге
                    "num_predict": 4096, # Ограничение для диалоговых ответов
                    "num_ctx": 200000,   # Большой контекст для учета истории диалога
                    "stop": ["<|im_end|>", "</s>", "<|endoftext|>", "Human:", "Assistant:", "User:"], # Расширенные stop-токены
                    "system": "Отвечай только на русском языке!"
                }
            )
