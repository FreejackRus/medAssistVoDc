...
class MedicalAssistant:
    ...
    def generate_services_for_step(self, step_text: str, step_title: str = "") -> List[Dict[str, str]]:
        ...
        system_prompt = (
            "Ты — медицинский эксперт. Твоя задача — выбрать релевантные медицинские услуги на основе текста.\n\n"
            "**ВАЖНО:**\n"
            "- Отвечай **только** валидным JSON-массивом.\n"
            "- Максимум 5 услуг.\n"
            "- Не добавляй объяснений, комментариев или текста вне JSON.\n"
            "- Формат ответа: [{\"id\": \"123\", \"name\": \"Название услуги\"}]\n"
            "- Если услуг нет — верни: []\n\n"
            f"Доступные услуги:\n{services_text}\n\n"
            f"Этап: {step_title}\n"
            f"Описание: {step_text[:2000]}"
        )
